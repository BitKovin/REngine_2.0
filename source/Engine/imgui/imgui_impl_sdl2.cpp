...
// [ENGINE] Engine hooks, used by the single-viewport path only (mouse position / screen size overrides).
...
// [ENGINE] <SDL_vulkan.h> is intentionally not included: bgfx owns Vulkan, and not every platform ships the header.
#if !SDL_HAS_VULKAN
...
bool ImGui_ImplSDL2_ProcessEvent(const SDL_Event* event)
{
    ...
    // [ENGINE] Single-viewport builds (Android/Web/...) feed Dear ImGui from the engine's own cursor.
    // With multi-viewports enabled ImGui needs absolute OS coordinates instead, which the stock code below provides.
    const bool engine_mouse = !(io.ConfigFlags & ImGuiConfigFlags_ViewportsEnable);
    if (engine_mouse)
    {
    ...
    switch (event->type)
    {
        case SDL_MOUSEMOTION:
        {
            ...
            if (!engine_mouse) // [ENGINE] engine cursor was already submitted above
...
static void ImGui_ImplSDL2_UpdateMouseData()
{
    ...
    if (is_app_focused)
    {
        ...
        if (bd->MouseCanUseGlobalState && bd->MouseButtonsDown == 0 && !is_relative_mouse_mode)
        {
            ...
            if (!(io.ConfigFlags & ImGuiConfigFlags_ViewportsEnable))
            {
                // [ENGINE] engine cursor instead of window-relative global mouse state
...
void ImGui_ImplSDL2_NewFrame()
{
    ...
    // [ENGINE] engine-defined screen size
    ...
    // [ENGINE] (SDL_Vulkan_GetDrawableSize branch removed, see the include block at the top)
    else
        SDL_GL_GetDrawableSize(bd->Window, &display_w, &display_h);

    // [ENGINE] framebuffer scale is always 1 (engine works in ScreenSize pixels)
    display_w = w;
    display_h = h;
    ...
    // [ENGINE] single-viewport builds: engine cursor (multi-viewport uses absolute OS coordinates, see UpdateMouseData)
    if (!(io.ConfigFlags & ImGuiConfigFlags_ViewportsEnable))
    {
        Input::UpdateMousePosition();
        io.AddMousePosEvent(Input::MousePos.x, Input::MousePos.y);
...
// [ENGINE] Vulkan surface creation (Platform_CreateVkSurface) removed: bgfx creates its own surfaces from the native window handle.
